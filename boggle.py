...
#    def make_board_without_very_many_vowels(self):
...
#        return board
    ...
    def make_board(self):
        """Make and return a random boggle board with increased vowel probability."""
        vowels = 'AEIOU'
        consonants = ''.join(set(string.ascii_uppercase) - set(vowels))
	                
        # Original probability of vowel: 5/26 ~ 0.1923
        # New probability: 0.1923 * 2 = 0.3846
        vowel_weight = 0.3846
        consonant_weight = 1 - vowel_weight
	
        board = []
        for y in range(5):
            row = [choices([vowels, consonants], weights=[vowel_weight, consonant_weight])[0] for i in range(5)]
            row = [choice(group) for group in row]
            board.append(row)
